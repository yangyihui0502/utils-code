...
def check_broadcast(a, b):
    ...
    mask = (torch.tensor(a.shape) != torch.tensor(b.shape))
    if mask.sum() <= 1:
        return
    else:
        if (torch.tensor(a.shape) * mask.float()).sum() == mask.sum():
            return
        if (torch.tensor(b.shape) * mask.float()).sum() == mask.sum():
            return
        raise ValueError


def check_broadcast_wolast2(a, b):
    assert len(a.shape) == len(b.shape)
    mask = (torch.tensor(a.shape[:-2]) != torch.tensor(b.shape[:-2]))
    if mask.sum() <= 1:
        return
    else:
        if (torch.tensor(a.shape[:-2]) * mask.float()).sum() == mask.sum():
            return
        if (torch.tensor(b.shape[:-2]) * mask.float()).sum() == mask.sum():
            return
        raise ValueError
...
def get_mat_BtoA(matA, matB):
    ...
    Args:
        matA (tensor): [..., 3, 3] world matrix
        matB (tensor): [..., 3, 3] world matrix
    """
    check_broadcast(matA, matB)
...
def get_mat_BfromA(matA, matBtoA):
    """
        return matB in world

    Args:
        matA (_type_): [..., 3, 3] matA in world
        matBtoA (_type_): [..., 3, 3] matB rel2 matA
    """
    check_broadcast(matA, matBtoA)
    if isinstance(matA, torch.Tensor):
...
def get_position_from(pos, mat):
    ...
    Args:
        pos (_type_): [N, M, 3] or [N, 3], pos rel2 transmat
        mat (_type_): [N, 4, 4] or [4, 4], transmat

    Returns:
        _type_: pos in world
    """
    check_broadcast_wolast2(pos, mat)
    if isinstance(mat, torch.Tensor):
...
def get_position_from_rotmat(pos, mat):
    ...
    Args:
        pos (_type_): [N, M, 3] or [N, 3], pos rel2 mat
        mat (_type_): [N, 3, 3] or [3, 3], mat
...
def get_relative_position_to(pos, mat):
    ...
    Args:
        pos (_type_): [N, M, 3] or [N, 3], pos in world
        mat (_type_): [N, 4, 4] or [4, 4], transmat in world
...
def get_relative_direction_to(dir, mat):
    ...
    Args:
        dir (_type_): [N, M, 3] or [N, 3], dir in world
        mat (_type_): [N, 4, 4] or [4, 4], transmat in world

    Returns:
        _type_: dir rel2 transmat
    """
    check_broadcast_wolast2(dir, mat)
    if isinstance(mat, torch.Tensor):
        mat_inv = torch.inverse(mat)
    elif isinstance(mat, np.ndarray):
        mat_inv = np.linalg.inv(mat)
...
def get_direction_from(dir, mat):
    ...
    Args:
        dir (_type_): [N, M, 3] or [N, 3], dir in transmat
        mat (_type_): [N, 4, 4] or [4, 4], transmat in world

    Returns:
        tensor: [N, M, 3] or [N, 3], dir in world
    """
    check_broadcast_wolast2(dir, mat)
    rot_mat = mat[..., :3, :3]
    if isinstance(mat, torch.Tensor):
...
def xzdir2rotmat(dir):
    """
        _summary_

    Args:
        dir (_type_): [N, 2], dir x, z
    ...
    if isinstance(dir, torch.Tensor):
        rotmat = torch.zeros(dir.shape[:-1] + (3, 3), device=dir.device)
        rotmat[..., 0, 0] = z
        rotmat[..., 0, 2] = x
        rotmat[..., 2, 0] = -x
        rotmat[..., 2, 2] = z
        rotmat[..., 1, 1] = 1.0
        rotmat = normalized_matrix(rotmat)
    elif isinstance(dir, np.ndarray):
        rotmat = np.zeros(dir.shape[:-1] + (3, 3))
        rotmat[..., 0, 0] = z
        rotmat[..., 0, 2] = x
        rotmat[..., 2, 0] = -x
        rotmat[..., 2, 2] = z
...
def calc_heading_quat(q, head_ind=0):
    # type: (Tensor, int) -> Tensor
