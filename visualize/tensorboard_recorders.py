...
# 可加上resume=False，即每次训练时清空当前文件夹内已有的训练数据
self.recorder = EgoMoGenTensorboardRecorder(record_dir=f'EgoMoGen_data/record/{opt.record_exp}')
    ...
    def update_scalar_stats(self, scalar_stats: dotdict):
        ...
        for k, v in scalar_stats.items():
            if isinstance(v, (int, float)):
                ...
                self.scalar_stats[k].update(sum(v)/len(v))
